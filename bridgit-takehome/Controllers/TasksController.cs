...
namespace Tasklify.Controllers
{
    ...
    public class TasksController : ControllerBase
    {
        ...
        public TasksController(IUsersDAL userDal, TaskRepository taskRepository)
        {
            _uDal = userDal;
        ...
        public async Task<IActionResult> Add([FromBody] TasklifyTask task)
        {
            try
            {
                ...
                var tmpTask = await _taskRepository.AddAsync(task.Summary, task.Description, task.Assignee);
                return Ok(tmpTask);
            }
        ...
        public async Task<IActionResult> UpdateById(int id, [FromBody] TasklifyTask task)
        {
            try
            {
                ...
                ValidateTask(task);
                return Ok(await _taskRepository.UpdateByIdAsync(id, task));
            }
            catch (FormatException fex)
            {
